...
store는 **UI가 즉시 필요한 클라이언트 상태**만 담는다. 서버 데이터·캐시는 TanStack Query에 둔다.

store에 넣는 것: 세션 한정 UI 상태
store에 넣지 않는 것: 서버에서 온 데이터, 다른 state에서 파생 가능한 값

### 액션은 메서드로, state 직접 수정 금지

컴포넌트·hook에서 state 필드를 직접 조작하지 않는다. store는 **의도가 드러나는 액션(메서드)** 만 외부에 노출한다.

- 한 액션 = 한 가지 사용자 의도 단위로 묶는다

### 읽기도 메서드로 캡슐화

store 내부 자료구조를 셀렉터에서 직접 꺼내지 않는다. store에 읽기 메서드를 두고 셀렉터에서 호출한다. 내부 구조가 바뀌어도 소비자 코드는 수정 불필요.
