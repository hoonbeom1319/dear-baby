...
export const AdminPlaces = ({ initialPlaces }: Props) => {
    ...
    const openEdit = (place: PlaceAdmin) => {
    ...
    if (mode !== 'list') {
        return (
            <AdminPage title={mode === 'edit' ? '장소 수정' : '새 장소 추가'} subtitle="운영자가 직접 채우는 데이터의 본진 · A-3">
                <button
                    type="button"
                    onClick={closeForm}
                    ...
                    <div className="flex flex-col gap-[18px] p-6">
                        ...
                        <div className="grid grid-cols-4 gap-4">
                            ...
                            <AdField label="권장 시작 월령">
                                <AdInput
                                    ...
                                    onChange={(e) => set('ageRangeStart', e.target.value)}
                                    placeholder="6"
                                    inputMode="numeric"
                                />
                            </AdField>
                            <AdField label="권장 끝 월령">
                                <AdInput
                                    ...
                                    onChange={(e) => set('ageRangeEnd', e.target.value)}
                                    placeholder="48 (없으면 빈칸)"
                                    inputMode="numeric"
                                />
                            </AdField>
                            <AdField label="표시 순서">
                        ...
                        <AdField label="한 줄 설명 · 큐레이터 코멘트">
                            <AdTextarea
                                ...
                                placeholder="첫 화면 카드와 상세화면에 같이 보여요"
                            />
                        </AdField>
    ...
    return (
        ...
        >
            ...
            <div className="overflow-hidden rounded-xl border border-border bg-surface">
                    ...
                    <tbody>
                        {filtered.map((place) => (
                            <tr key={place.id} className="transition-colors hover:bg-neutral-50">
                                <td className="border-b border-border px-4 py-3.5 text-surface-foreground tabular-nums">{place.sortOrder}</td>
                                ...
                                <td className="border-b border-border px-4 py-3.5">
                                    <div className="flex items-center gap-1">
                                        {Array.from({ length: 5 }).map((_, j) => (
                                            <span
                                                key={j}
                                                className={cn('h-2 w-2 rounded-sm', j < place.amenities.length ? 'bg-primary-500' : 'bg-neutral-200')}
                                            />
                                        ))}
                                ...
                                <td className="border-b border-border px-4 py-3.5">
                                    <button type="button" onClick={() => handleToggleStatus(place)}>
                                        <Pill tone={place.status === 'public' ? 'success' : 'warning'}>{place.status === 'public' ? '공개' : '검토중'}</Pill>
                                    </button>
                                </td>
                                <td className="border-b border-border px-4 py-3.5">
                ...
                {filtered.length === 0 && <div className="py-10 text-center text-[13px] text-muted">해당 조건에 장소가 없어요</div>}
            </div>
