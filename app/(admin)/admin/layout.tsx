...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
    return (
        <div className="flex min-h-dvh w-full text-neutral-900">
            <aside className="w-72 shrink-0 border-r border-neutral-200 bg-white p-6">
                <div className="mb-10 flex items-center gap-3">
                    ...
                    <div className="flex flex-col">
                        <div className="text-sm leading-none font-bold">육아똑똑 관리자</div>
                        <div className="mt-1 text-xs font-medium text-neutral-500">Admin Console</div>
                    </div>
                </div>

                <nav className="flex flex-col gap-2">
                    <Link className="rounded-xl px-4 py-3 text-sm font-semibold text-neutral-800 hover:bg-neutral-100" href="/admin">
                        대시보드
                    </Link>
                    <Link className="rounded-xl bg-orange-50 px-4 py-3 text-sm font-bold text-orange-900" href="/admin/places">
                        장소 관리
                    </Link>
                </nav>

                <button className="mt-10 w-full rounded-xl bg-neutral-100 py-3 text-sm font-bold text-neutral-700 hover:bg-neutral-200" type="button">
                    로그아웃
                </button>
            </aside>
